...
const sections = [
  {
    ...
    body: [
      <>Monitoring tools like Datadog give your engineering teams <strong>deep, real-time visibility</strong> into cloud infrastructure: metrics, traces, logs, synthetic tests, all in one place. That visibility is invaluable for debugging and capacity planning. But there's a parallel audience that needs to understand your infrastructure health too: <strong>your stakeholders</strong>.</>,
      <>In mature organisations, staying ahead of incidents is only half the battle. <strong>The other half is communication.</strong> When services degrade, the questions come fast: from product managers, from account teams, from executives. A well-maintained status page <strong>answers those questions before they're even asked</strong>. It replaces ad-hoc Slack announcements, email chains, and manual maintenance notices with a <strong>single, authoritative, self-service source of truth</strong> that anyone can subscribe to.</>,
    ],
    highlights: [
  ...
  },
  {
    ...
    highlights: [
      "Workflow Automations support both monitor-based and synthetic-based triggers, so you can chain both into the same status page update workflow.",
  ...
  },
  {
    ...
    highlights: [
      "Use AND (&&) logic in composites to require multiple failure signals before declaring an outage. That prevents noise-driven false positives.",
  ...
  },
  {
    ...
    body: [
      <>The status page is excellent for real-time communication, but it has a hard limitation: <strong>the incident history only reaches back 90 days</strong>. That's not useful when an executive asks for year-to-date uptime, or when you need to demonstrate SLA compliance over a contract period.</>,
      <>The solution is to build a separate SLO dashboard in Datadog, but this comes with its own complexity. Datadog SLOs come in two flavours: <strong>monitor-based</strong> and <strong>metric-based</strong>. Monitor-based SLOs are easy to create but difficult to combine accurately. If you group three monitor-based SLOs into a parent SLO, <strong>Datadog defaults to OR logic</strong>, meaning the parent SLO fails if any single sub-SLO fails. For a service with redundant sub-components this produces misleading numbers, and the only manual escape valve is the <strong>corrections feature</strong>, which requires ongoing human intervention to apply. That's the opposite of zero-maintenance.</>,
    ],
    highlights: [
      "Datadog composite SLOs use OR logic by default. That's correct for some architectures but wrong for redundant systems.",
      ...
      <div className="max-w-3xl mx-auto px-6 py-24">
        <div className="mb-12">
          ...
          <p className="text-muted-foreground text-lg leading-relaxed">
            ...
            reliability requires an extra layer of engineering. Here's how we built a fully automated status page and a
